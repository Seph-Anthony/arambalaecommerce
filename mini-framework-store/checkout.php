...
<div class="container my-5 checkout-page-wrapper">
    ...
    if (isset($_SESSION['message'])) {
        ...
        if (strpos($_SESSION['message'], 'Error') !== false) $alertType = 'danger';
        if (strpos($_SESSION['message'], 'fill in all') !== false) $alertType = 'warning'; // For validation errors

        echo '<div class="alert alert-' . $alertType . ' text-center mb-4 fade show" role="alert">' . htmlspecialchars($_SESSION['message']) . '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>';
